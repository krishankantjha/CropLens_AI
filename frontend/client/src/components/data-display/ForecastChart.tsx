...
type ForecastChartProps = { points: ForecastPoint[]; loading?: boolean };



type ChartPoint = ForecastPoint & { label: string; low: number | null; mid: number; high: number | null; peak: boolean; today: boolean };
...
export const ForecastChart = memo(function ForecastChart({ points, loading = false }: ForecastChartProps) {
  ...
  const usable: ChartPoint[] = points
    ...
    .map((point, index) => {
      ...
      return mid === null ? null : {
        ...
        label: point.day_index === 0 || point.type === "current"
          ? t("chartToday")
          : language === "hi"
            ? point.day_name_hi ?? point.day_name ?? point.day ?? point.date ?? `${t("day")} ${point.day_index ?? index}`
            : point.day_name ?? point.day ?? point.date ?? `${t("day")} ${point.day_index ?? index}`,
        ...
        today: point.day_index === 0 || point.type === "current",
  ...
  const today = usable.find((point) => point.today) ?? usable[0];



  return (

    <div className={`chart-wrap${loading ? " chart-wrap--loading" : ""}`} aria-busy={loading}>

      <ResponsiveContainer width="100%" height={280}>

        <AreaChart data={usable} margin={{ top: 20, right: 18, left: 4, bottom: 8 }}>
          ...
          <Tooltip
            ...
            content={({ active, payload }) => {
              ...
              const dayTag = point.today ? ` · ${t("chartToday")}` : point.peak ? ` · ${t("chartBestDay")}` : "";

              return (

                <div className="chart-tooltip">

                  <strong>{point.label}{dayTag}</strong>

                  <span>{t("priceLow")}: {money(point.low)}</span>

                  <span>{t("priceMid")}: {money(point.mid)}</span>

                  <span>{t("priceHigh")}: {money(point.high)}</span>
          ...
          <Area
            ...
            dot={(props) => {
              ...
              if (cx === undefined || cy === undefined) return <circle cx={0} cy={0} r={0} fill="none" />;

              if (payload?.today) {

                return <circle cx={cx} cy={cy} r={6} fill="var(--green)" stroke="var(--paper)" strokeWidth={3} />;

              }

              return <circle cx={cx} cy={cy} r={4} fill="var(--paper)" stroke="var(--green)" strokeWidth={2} />;
          ...
          {today.label !== peak.label ? (
            ...
            <ReferenceDot
              ...
              label={{ value: t("chartToday"), position: "bottom", fill: "var(--green)", fontSize: 11, fontWeight: 700 }}
          ...
          <ReferenceDot
            ...
            label={{ value: "★", position: "top", fill: "var(--amber)", fontSize: 16 }}
      ...
      <p className="sr-only">
        ...
        {t("chartToday")}: {today.label}. {t("peakDayIs")} {peak.label}. {t("chartExplanation")}
