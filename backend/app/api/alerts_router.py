...
class SubscribeAlertRequest(BaseModel):
    mobile_number: str = Field(..., json_schema_extra={"example": "9876543210"})
    channel: str = Field(default="whatsapp", json_schema_extra={"example": "whatsapp"})
    crop: str = Field(default="Potato", json_schema_extra={"example": "Potato"})
    mandi: str = Field(default="Agra", json_schema_extra={"example": "Agra"})
...
def subscribe_alert_endpoint(
    ...
    """Saves or updates a farmer's daily WhatsApp alert subscription in SQLite."""
    ...
    delivery_time = normalize_delivery_time(req.delivery_time)
    ...
    if existing:
        ...
        return {
            ...
            "message": f"Updated WhatsApp advisory subscription for {req.crop} ({req.mandi}) at {delivery_time}."
        }
    ...
    return {
        ...
        "message": f"Daily WhatsApp advisory activated for {req.crop} ({req.mandi}) at {delivery_time}."
    }
...
def dispatch_now_endpoint(request: Request, _: User = Depends(require_system_operator), __: None = Depends(verify_csrf)) -> Dict[str, Any]:
    ...
    """
    return dispatch_scheduled_advisories_service(request.app, respect_delivery_time=False)
