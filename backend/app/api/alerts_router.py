...
class SendWhatsappAdvisoryRequest(BaseModel):
    ...
    crop: str = Field(default="Potato", example="Potato")
    mandi: str = Field(default="Agra", example="Agra")
    decision: str = Field(default="HOLD FOR 5 DAYS", example="HOLD FOR 5 DAYS")
    current_price: float = Field(default=1650.0, example=1650.0)
    target_price: float = Field(default=1780.0, example=1780.0)
    expected_gain: float = Field(default=130.0, example=130.0)
    lang: str = Field(default="hi", example="hi")
...
class SubscribeAlertRequest(BaseModel):
    ...
    channel: str = Field(default="whatsapp", example="whatsapp")  # "whatsapp", "telegram", "both"
    crop: str = Field(default="Potato", example="Potato")
    mandi: str = Field(default="Agra", example="Agra")
    delivery_time: str = Field(default="07:00 AM", example="07:00 AM")
    language: str = Field(default="hi", example="hi")


class TestTelegramRequest(BaseModel):
    chat_id: str = Field(..., example="123456789")
    crop: str = Field(default="Potato", example="Potato")
    mandi: str = Field(default="Agra", example="Agra")
    lang: str = Field(default="hi", example="hi")
...
def send_whatsapp_advisory_endpoint(req: SendWhatsappAdvisoryRequest) -> Dict[str, Any]:
    ...
    Formats advisory and returns live sandbox dispatch details + 1-click wa.me deep-link URL.
    """
...
def subscribe_alert_endpoint(req: SubscribeAlertRequest) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        existing = db.query(AlertSubscription).filter(
            AlertSubscription.mobile_number == req.mobile_number,
...
def list_subscriptions_endpoint(mobile_number: Optional[str] = None) -> Dict[str, Any]:
    """Lists registered active alert subscriptions."""
    db = SessionLocal()
    try:
        query = db.query(AlertSubscription)
        if mobile_number:
            query = query.filter(AlertSubscription.mobile_number == mobile_number)
        subs = query.order_by(AlertSubscription.created_at.desc()).all()
        return {
            "status": "success",
...
def delete_subscription_endpoint(subscription_id: int) -> Dict[str, Any]:
    """Deactivates/deletes an alert subscription."""
    db = SessionLocal()
    try:
...
def get_alert_logs_endpoint(limit: int = 50) -> Dict[str, Any]:
